...
union isa_t {
    //c++无参构造函数
    isa_t() { }
    //c++的含有一个参数的构造函数,uintptr_t为typedef过的usigned long数据类型,为64位
    isa_t(uintptr_t value) : bits(value) { }
//下方的cls为一般的指向类对象的指针,用于找到对象所属的类，说白了就是个8byte的指针
    Class cls;
//64位的数据类型bits和ISA_BITFILED里边的定义的位域相结合，在——86平台的64字节的前提下，这ISA_BITFILED的大小也为64位，并且一个指针在64位（x86）的基础上也是64位的数据类型，可以和class类型的数据相对应
    uintptr_t bits;
