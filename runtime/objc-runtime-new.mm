...
下方的cls->对应的函数均是对上方bits的操作,注意objc_object结构体和objc_class中均有bits属性，但是两个的定义不一致，在objc_object中为isa_t的union结构中的uintptr_t bits属性,而objc_class中的bits则为上方所示
     */
//取class_data_bits_t中的4到47位的class_rw_t的结构，然后再取class_rw_t中的flag项数据的倒数第18位的判断标志
    bool hasCxxCtor = cls->hasCxxCtor();
    //上下两个函数均为利用掩码的按位与操作，下方和上方一样，不过最后是取倒数第17位。
    bool hasCxxDtor = cls->hasCxxDtor();
    bool fast = cls->canAllocNonpointer();
//extraBytes为0
