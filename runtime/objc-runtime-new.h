...
public:
    ...
    class_rw_t* data() {
        //这里的FAST_DATA_MASK也为取第4位至第47位的二进制信息
...
#else
    bool hasDefaultAWZ() {
        //data()返回4——47位域的信息，在和RW_HAS_DEFAULT_AWZ做按位于操作，取倒数地16位的位的信息
    ...
    bool instancesRequireRawIsa() {
        //data()的操纵为用bits和一个掩码进行按位与操作，取倒数低15位
    ...
    size_t fastInstanceSize() 
    {
        ...
        //FAST_SHIFTED_SIZE_SHIFT为52，bites类型为usigned long，故这一步为64位右移52位，剩下12位，这剩下的12位应该是包含了对象大小的信息（二进制）.
...
struct objc_class : objc_object {
    ...
    class_rw_t *data() {
        //下方是取4到47位的信息，并且做了（class_rw_t *）的转换
        return bits.data();
    }
    void setData(class_rw_t *newData) {
    ...
    uint32_t alignedInstanceSize() {
        /*
         在unalignedInstanceSize()中主要是取得这个值：
         uint32_t instanceSize
         其位于
         struct class_ro_t {
         uint32_t flags;
         ...
         */
        /*
这个值是有编译器根据你的类的定义来写入值的，如果有定义实例变量--及通过@property由编译器定义出来的实例变量，还是用{}的方式定义的实例变量，都会计算出来相应的值，保存在这个class_ro_t结构体的uint32_t instanceSize;这一项中。
         **/
