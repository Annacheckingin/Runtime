...
objc_object::ISA() 
{
    //如果非taggedpointer那么就警告
    assert(!isTaggedPointer());
    //下方的编译器指令为假如支持indeded_isa的话,即是否支持taggedpointer
#if SUPPORT_INDEXED_ISA
    //
//SUPPORT_INDEXED_ISA-在arm64位的情况下为1，否则为0，说白了就是支持64位的指针与否，这很重要，64位的情况下，你才能在其中载入更多的信息,但是不是说64位你就一定放入其他的信息，有可能还是和普通指针一样，并没有放入其他的信息
    //如果不是taggedpointer
//这里的nonpointer是位于ISA_BITFIELD宏定义中的一个位域，位于第一个位域，他才是决定次指针（即使是64位的）到底有没有载入更多的信息,nonpointer=not normal pointer,为1则h包含更多的信息。
    if (isa.nonpointer)
    {
        //isa.indexcls为15位的位域
    ...
    //假如不支持indexedisa的话,一般情况下是走的这里,这里的bits为一个结构体
    //ISA_MASK        0x00007ffffffffff8ULL
    //isa为一个isa_t结构体,结构体中含有bits属性，bits为
    //  uintptr_t bits
    //typedef unsigned long           uintptr_t;
    //这里的返回值为一个指针，由于并没有采用位域存储更多的信息，所以直接返回bits(isa_t中唯一的数据项)
    return (Class)isa.bits;
#else
//非64位系统，ISA_MASK为取第4位到第47位到二进制位，这里的isa位objc_object中的isa指针（或者说是共同体）,不要忘了，类本身也是一个对象。这里的bits是一个isa_t结构体，可以这么说-不管是32位还是64位都是isa_t结构体的isa指针，只是oc1.0版本里边才是Class类型的is指针
    //就是取出64位中一定范围的位域信息--真正的信息
...
{
    //这时，这里的cls为class类对象的指针
    //如果instancesRequireRawIsa()返回位真则assert
    assert(!cls->instancesRequireRawIsa());
    //取得从类对象中的是否有C++构析函数的标识位
...
objc_object::initIsa(Class cls, bool nonpointer, bool hasCxxDtor) 
{
    //nonpointer位true
    assert(!isTaggedPointer()); 
//根据nonopointer进行判断来进行逻辑的跳转，在整个软件周期中if-else会交替执行非常多次，差别就是如果支持联合体的isa指针则走else分支，否则走第一个分支，这里直接会跳过，因为nonpointer为1
    if (!nonpointer)
    {
//相当于是将object内部的isa的cls变量赋于为cls,即将对象的isa指针指向类对象,就是个没有附加信息的普通指针时
        isa.cls = cls;
    ...
    } else
    {
        ...
        assert(!cls->instancesRequireRawIsa());
      //给isa_t联合体中的bits属性进行赋值0 ，那么这个64位的bits信息都为0，可视作初始化
        isa_t newisa(0);
     
#if SUPPORT_INDEXED_ISA
        //如果支持64位的arm系统
        assert(cls->classArrayIndex() > 0);
        //在isa.h中，define ISA_INDEX_MAGIC_VALUE 0x001C0001，即第1、19、20、21位都为1
        ...
        // isa.nonpointer is part of ISA_MAGIC_VALUE
        newisa.has_cxx_dtor = hasCxxDtor;
        //将类的地址信息存在shitcls区域中,类对象的地址低三位本来就为0，所以可以清除然后赋值给这个区域的位域
...
//最后进行赋值
        ...
        //最后讲将这个刚初始化的对象的isa指针赋值为newisa的值;
        isa = newisa;
    }
}
