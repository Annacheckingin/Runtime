...
{
    //这里默认的checkNill为false,并且allocWithZone为ture,所以这里的判断一般直接跳过
    if (slowpath(checkNil && !cls)) return nil;
//这里的hasCustomAWZ()这个函数存在于 objc_class中，这个结构体是继承自objc_object结构体的
#if __OBJC2__
    //对于类对象来说，调用isa()就是找出元类的位置,元类也是一个类，所以可以调用hasCustomAWZ()函数,对象结构体中的有private的isa_t类型的isa项，这个union类型中有个bits项，在继承自对象结构体\Class类型的结构体中（很明显这个结构体是用来描述类的本身的信息的）也存在一个bits项，不过是class_data_bits_t类型的，由于Class结构体继承自Objc_object类型（即对象类型），所以可以调用ISA()函数-不过返回的是元类的位置，元类调用hasCustomAWZ()函数
    ...
    if (fastpath(!cls->ISA()->hasCustomAWZ()))
    {
        ...
        //下方默认位false，
        //是因为objc_class结构体中的class_data_bits_t bits--这个数据项的的canAllocFast()(由objc_class的canAllocFast()调用的bits的canAllocFast())返回值为false
        if (fastpath(cls->canAllocFast()))
        {
            
            // No ctors, raw isa, etc. Go straight to the metal.
            bool dtor = cls->hasCxxDtor();
            //bits就是class_data_bits_t类型的数据项，fastInstanceSize()内部位abort函数
